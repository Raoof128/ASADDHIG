...
class AustralianPIIInspector:
    ...
    def __init__(self):
        ...
        self.sensitive_keywords = [
            'medicare', 'tfn', 'tax file', 'driver licence', 'drivers license',
            'passport', 'credit card', 'bank account', 'bsb', 'account number',
            'diagnosis', 'patient', 'medical record', 'prescription', 'medication',
            'legal advice', 'court case', 'criminal', 'conviction',
            'salary', 'income', 'superannuation', 'super', 'pension',
            'australian security', 'classified', 'confidential'
        ]
    ...
    def _compile_patterns(self) -> Dict[str, re.Pattern]:
        ...
        return {
            ...
            'medicare': re.compile(
                r'\b\d{4}\s?\d{3}\s?\d{3}\b|\b\d{10}\b',
                re.IGNORECASE
            ),
            ...
            'drivers_licence': re.compile(
                r'\b(?:DL|LIC|LICENCE|LICENSE)[\s:]*([A-Z]\d{6,8}|\d{8})\b',
                re.IGNORECASE
            ),
